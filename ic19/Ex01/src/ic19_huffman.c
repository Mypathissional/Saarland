...
TREE* build_huffman_tree(
  ...
  while (h->size > 0) {
    ...
    node.left = &(htree->nodes[x.id]);
    node.right = &(htree->nodes[y.id]);
...
void encode_huffman(
  ...
  for (i=0;i<s;i++) {
    if (counter[i]>0) {
      ...
      /*OUR CODE*/
      symbolmap[i] = symbolcount;
      histogram[symbolcount] = symbolmap[i];
      /*END OF OUR CODE*/
  ...
  for (i=0;i<n;i++) {
    ...
    for (j=0;j<lengths[sourceword[i]];j++) {
      ...
      /* TODO: write next bit of binary code stored in "bcode". 
         ...
         /*OUR CODE*/
         /*TODO: CHECK IF WE AREN'T WRITING THE CODE BACKWARDS*/
         set_bit(compressed, bcode%2);
         bcode = bcode/2;
         /*END OF OUR CODE*/
    }
    if (debug_file != 0) {
