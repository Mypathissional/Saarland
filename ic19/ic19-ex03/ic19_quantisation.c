...
void prepare_uniform_maps(long n, /* number of quantised values */
  ...
  for (i = 0; i < max_q; i++) {
    ...
    quantisation_map_small[i] = floor(i*n/max_q);
    /*printf("%ld -> %ld\n",i,quantisation_map_small[i]);*/
  }
  /* TODO: special case for last value */
  quantisation_map_small[max_q] = n-1;
  ...
  for (i = 0; i < n; i++) {
    ...
    quantisation_map_back[i] = floor((i+0.5)*max_q/n);
  ...
  for (c = sc; c < nc; c++)
    for (x = bx; x < nx + bx; x++)
      for (y = by; y < ny + by; y++) {
        ...
        quantised[c][x][y] = (float) quantisation_map_small[(long)original[c][x][y]];
      }
}
...
void quantise_image_kmeans(float*** original,
  ...
  while (changes > 0) {
    ...
    for (i=0; i<colour_count; i++) {
      ...
      */
      cindex = find_closest_cluster(colours[i],centres,n);
      if(cindex != quantisation_map[i]) {
        quantisation_map[i] = cindex;
        changes++;
      }
  ...
  for (i = 1; i <= nx; i++) {
    for (j = 1; j <= ny; j++) {
      ...
      printf("%ld -> %ld -> (%ld %ld %ld)\n",cindex,
        ...
        r = (long)original[0][i][j];
        g = (long)original[1][i][j];
        b = (long)original[2][i][j];
        cindex = quantisation_map[index_map[r][g][b]];
        quantised[0][i][j] = (float)centres[cindex].r;
        quantised[1][i][j] = (float)centres[cindex].g;
        quantised[2][i][j] = (float)centres[cindex].b;
